...
def chaos_node_two_validator(chaos_node_two_validator_build):
    ...
    # up to one minute.
    address = chaos_node_two_validator_build["gen_nodes"]("2 --rpc-address | head -n 1")
    upcheck_interval = 2
    for attempt in range(0, 60, upcheck_interval):
        run_status = run_script.poll()
        if run_status is not None:
            raise Exception(f"run_many.sh exited unexpectedly with code {run_status}")
    ...
    try:
        yield dict(**chaos_node_two_validator_build, run=run_cmd)
    finally:
        print("chaos_node_two_validator fixture: running stop_many.sh")
        run_cmd(os.path.join(chaos_node_two_validator_build["scripts"], "stop_many.sh"))
...
def two_chaos_nodes_and_tool(chaos_node_two_validator, chaostool_build):
    ...
    ndauhomes = chaos_node_two_validator["run"](
        f'find {chaos_node_two_validator["multinode"]}' " -type d -name ndau -print"
    ).splitlines()

    env = {"PATH": os.environ["PATH"], "NDAUHOME": ndauhomes[0]}
    ...
    subp(
        # TODO: Once we stop skipping two-node tests, we should add an --ndau argument
